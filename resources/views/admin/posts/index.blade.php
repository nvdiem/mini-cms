  ...
  <div class="card overflow-hidden">
    <div class="px-4 sm:px-6 py-3 bg-slate-50/70 dark:bg-slate-800/30 border-b border-border-light dark:border-border-dark">
      <div class="flex flex-col lg:flex-row gap-3 lg:items-center">
        {{-- Bulk Actions --}}
        <form id="bulkForm" class="flex flex-col sm:flex-row gap-2 sm:items-center" method="POST" action="{{ route('admin.posts.bulk') }}">
          @csrf
        ...
        {{-- Search & Filters --}}
        <form class="flex flex-col sm:flex-row gap-2 sm:items-center flex-1" method="GET" action="{{ route('admin.posts.index') }}">
          <input type="hidden" name="trash" value="{{ request('trash') }}"/>
