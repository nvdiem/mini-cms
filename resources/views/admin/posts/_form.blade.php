  ...
  <div class="space-y-4">
    ...
    <div class="card p-6">
      <div class="flex items-center justify-between gap-3">
        ...
        <a class="text-sm text-primary hover:underline" href="{{ route('admin.tags.index') }}">Manage</a>
      </div>

      @if(($tags ?? collect())->count() === 0)
        <p class="mt-3 text-sm text-text-muted dark:text-slate-400">No tags yet. Create one first.</p>
      @else
        <div class="mt-3 space-y-2 max-h-56 overflow-auto pr-1">
          @foreach($tags as $tag)
            <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
