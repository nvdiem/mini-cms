<x-admin.layout :title="'Categories · Mini CMS'" :crumb="'Categories'">
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <div>
      <h1 class="text-2xl font-semibold text-text-strong dark:text-white tracking-tight">Categories</h1>
      <p class="text-sm text-text-muted dark:text-slate-400 mt-1">Create and manage categories.</p>
    </div>
  </div>

  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    <div class="card p-6">
      <div class="text-sm font-semibold text-text-strong dark:text-white">New category</div>
      <form class="mt-4 space-y-3" method="POST" action="{{ route('admin.categories.store') }}">
        @csrf
        <div>
          <label class="text-sm font-medium">Name</label>
          <input class="input mt-1" name="name" value="{{ old('name') }}" placeholder="Tech" required/>
          @error('name') <div class="text-xs text-red-600 mt-1">{{ $message }}</div> @enderror
        </div>
        <div>
          <label class="text-sm font-medium">Slug (optional)</label>
          <input class="input mt-1" name="slug" value="{{ old('slug') }}" placeholder="tech"/>
    ...
    <div class="card lg:col-span-2 overflow-hidden">
      <div class="card-hd">
        <form class="flex gap-3 items-center" method="GET" action="{{ route('admin.categories.index') }}">
          <div class="relative w-full sm:w-80">
            <input class="input pr-10" name="q" value="{{ $q }}" placeholder="Search categories..." />
          ...
          <a class="btn-soft px-3 py-2" href="{{ route('admin.categories.index') }}">Clear</a>
        </form>
      </div>
      ...
      @if($categories->count() === 0)
        <div class="p-10 text-center">
          ...
          <h2 class="mt-4 text-lg font-semibold text-text-strong dark:text-white">No categories yet</h2>
          <p class="mt-1 text-sm text-text-muted dark:text-slate-400">Create your first category using the form.</p>
        </div>
      @else
          ...
          <table class="table">
            ...
            <tbody class="divide-y divide-border-light dark:divide-border-dark">
              @foreach($categories as $cat)
                <tr class="row">
                  <td class="td">
                    <div class="font-medium text-text-strong dark:text-white">{{ $cat->name }}</div>
                    <div class="text-xs text-text-muted dark:text-slate-400">#{{ $cat->id }}</div>
                  </td>
                  <td class="td text-slate-600 dark:text-slate-300">{{ $cat->slug }}</td>
                  <td class="td text-right">
                    <details class="relative inline-block">
                      ...
                      <div class="absolute right-0 mt-2 w-72 rounded-xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark shadow-soft2 overflow-hidden z-10 p-3">
                        <form class="space-y-2" method="POST" action="{{ route('admin.categories.update', $cat) }}">
                          @csrf
                          @method('PUT')
                          <div>
                            <label class="text-xs font-medium text-slate-600 dark:text-slate-300">Name</label>
                            <input class="input mt-1" name="name" value="{{ $cat->name }}"/>
                          </div>
                          <div>
                            <label class="text-xs font-medium text-slate-600 dark:text-slate-300">Slug</label>
                            <input class="input mt-1" name="slug" value="{{ $cat->slug }}"/>
                          </div>
