<x-admin.layout :title="'Lead Detail · Mini CMS'" :crumb="'Lead Detail'">
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <div>
      <h1 class="text-2xl font-semibold text-text-strong dark:text-white tracking-tight">Lead #{{ $lead->id }}</h1>
      <p class="text-sm text-text-muted dark:text-slate-400 mt-1">Submitted {{ $lead->created_at->format('M j, Y \a\t g:i A') }}</p>
    </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    <div class="card lg:col-span-2 p-6">
      <div class="space-y-4">
        <div>
          ...
          <div class="mt-1 text-base text-text-strong dark:text-white">{{ $lead->name }}</div>
        </div>

        <div>
          <div class="text-xs font-semibold text-text-muted dark:text-slate-400 uppercase tracking-wider">Email</div>
          <div class="mt-1 text-base text-text-strong dark:text-white">
        ...
        @if($lead->phone)
          <div>
            <div class="text-xs font-semibold text-text-muted dark:text-slate-400 uppercase tracking-wider">Phone</div>
            <div class="mt-1 text-base text-text-strong dark:text-white">
        ...
        <div>
          <div class="text-xs font-semibold text-text-muted dark:text-slate-400 uppercase tracking-wider">Message</div>
          <div class="mt-2 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg border border-border-light dark:border-border-dark">
            <p class="text-sm text-text dark:text-slate-200 whitespace-pre-wrap">{{ $lead->message }}</p>
          </div>
        </div>

        <div>
          <div class="text-xs font-semibold text-text-muted dark:text-slate-400 uppercase tracking-wider">Source</div>
          <div class="mt-1 text-sm text-text dark:text-slate-300">{{ ucfirst(str_replace('_', ' ', $lead->source)) }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div class="card p-6">
        <div class="text-sm font-semibold text-text-strong dark:text-white">Details</div>
        <div class="mt-4 space-y-2 text-sm">
          <div class="flex justify-between">
            <span class="text-text-muted dark:text-slate-400">ID</span>
            <span class="text-text-strong dark:text-white font-medium">#{{ $lead->id }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-text-muted dark:text-slate-400">Submitted</span>
            <span class="text-text-strong dark:text-white">{{ $lead->created_at->format('M j, Y') }}</span>
          </div>
        </div>
      </div>
    </div>
