<x-admin.layout :title="'Support Inbox · Mini CMS'" :crumb="'Support Inbox'">
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <div>
      <h1 class="text-2xl font-semibold text-text-strong dark:text-white tracking-tight">Support Inbox</h1>
      <p class="text-sm text-text-muted dark:text-slate-400 mt-1">Manage visitor support conversations.</p>
    </div>
  </div>
  ...
  <div class="card overflow-hidden">
      ...
      <div class="flex flex-col lg:flex-row gap-3 lg:items-center">
        ...
        <form class="flex flex-col sm:flex-row gap-2 sm:items-center flex-1" method="GET" action="{{ route('admin.support.index') }}">
          <div class="relative w-full sm:w-64">
            <input class="input pr-10" name="q" value="{{ $q }}" placeholder="Search name, email, message..." />
          ...
          <a class="btn-soft px-3 py-2 sm:w-auto text-center" href="{{ route('admin.support.index') }}">Clear</a>
        </form>
    ...
    @if($conversations->count() === 0)
      <div class="p-10 text-center">
        ...
        <h2 class="mt-4 text-lg font-semibold text-text-strong dark:text-white">No conversations found</h2>
        <p class="mt-1 text-sm text-text-muted dark:text-slate-400">Adjust filters or wait for new visitor messages.</p>
      </div>
    @else
      {{-- Mobile Cards --}}
            ...
            @foreach($conversations as $conv)
              <tr class="row group cursor-pointer" onclick="window.location='{{ route('admin.support.show', $conv->id) }}'">
                ...
                <td class="td text-right">
                  <a href="{{ route('admin.support.show', $conv->id) }}" class="btn-soft px-3 py-1.5">View</a>
                </td>
              </tr>
            @endforeach
          </tbody>
